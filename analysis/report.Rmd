...
### Difference in Civilisation Win Rate from the Mean by ELO {.tabset .tabset-fade .tabset-pills}


```{r, echo = FALSE, results = "asis"}
cl <- readRDS("./data/g_ia_slice.Rds")
for ( i in names(cl)){
    cat("  \n  \n")
    cat("#### ", i)
    cat("  \n  \n")
...
```{r, echo = FALSE, results = "asis"}
cl <- readRDS("./data/g_iae12_cvc_civs.Rds")
for ( i in names(cl)){
    cat("  \n  \n")
    cat("#### ", i)
    cat("  \n  \n")
...
```{r, echo = FALSE, results = "asis"}
odat <- readRDS("./data/g_iae12_cvc_opt.Rds") %>%
    filter(p != 0) %>%
